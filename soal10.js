...
async function getProductsAndSuppliers() {
    console.time("Waktu Paralel");

    const productsPromise = fetchProductsFromAPI();

    const supplierPromises = [1, 3, 5].map((id) => {
        return fetchSupplierById(id);
    });

    const [productsJSON, suppliers] = await Promise.all([
        productsPromise,
